...
| AC-011 push notification | PARTIAL | Payload/deep-link/service-worker tests, browser subscription flow, persisted per-category preferences and server-side VAPID adapter tests pass; VAPID delivery and background-device test need quality credentials/device. |
...
| AC-013 local worker | PARTIAL | Registry, heartbeat, capability and concurrency tests pass; one-time registration-token issuance, server-side credential exchange, daemon control-plane client, authenticated heartbeat and pg-boss pull composition are available; Ollama/LM Studio network smoke, eligible-task assignment and offline reassignment require a configured local runtime/quality node. |
