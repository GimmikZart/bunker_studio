...
| AC-007 budget hard stop | PASS | Workflow and web route tests confirm provider handler is not invoked when estimate exceeds remaining budget; the task is persisted as `BLOCKED` or `WAITING_BUDGET_APPROVAL` and a tenant notification is emitted. |
