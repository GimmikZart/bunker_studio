...
| AC-013 local worker | PARTIAL | Registry, deterministic capability/scope/concurrency scheduler, heartbeat tests, one-time registration-token issuance, server-side credential exchange, daemon control-plane client, authenticated heartbeat and pg-boss pull composition are available; Ollama/LM Studio network smoke and end-to-end assignment/offline reassignment still require a configured local runtime/quality node. |
