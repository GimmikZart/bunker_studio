...
- `supabase db reset --local`: PASS; migrations `00000000000000..00000000000018` applicate, inclusi RLS assignments, trigger domain events, metadati workflow plan, worker registration RPC, stato delivery push, task claim/lease locali, outbox transazionale per task `QUEUED` e budget/report schedules.
- Test mirati post-hardening: Lead workflow plan 2/2, AgentRuntime 6/6, Anthropic adapter 2/2, Git 7/7, Notifications 4/4, Worker 10/10, Web routes 35/35 (inclusi archivio conversazioni, budget/design/task gates e chat budget/ledger), DB worker scheduler 3/3, typecheck 15/15: PASS.
