-- Preserve the task-state enum when the preflight transitions a queued task.
...
begin
  with exceeded as (
    ...
    set state = case
        when exceeded.hard_block then 'BLOCKED'::public.task_state
        else 'WAITING_BUDGET_APPROVAL'::public.task_state
      end,
